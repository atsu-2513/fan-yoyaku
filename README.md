...
### 毎週同じパターンで空き時間を開放したい場合

ある週の空き時間を設定したあと、その週の画面内にある「この週の設定を来週にコピー」ボタンを押すと、
同じ曜日・同じ時間の組み合わせが翌週にもコピーされる（例: 今週の火曜10-12時を開放していれば、来週の火曜10-12時も開放される）。
自動で毎週繰り返されるわけではなく、スタッフが必要なタイミングでボタンを押す方式。
